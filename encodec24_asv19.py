...
for i, audio_file in enumerate(audio_file_paths):
    ...
    # save spoof audio **
    torchaudio.save(f"{path_spoof}/E01_01_wav/E01_01_19D_{i:07}.wav", audio_values.squeeze(0), processor.sampling_rate)

    # metadata **
    filename = audio_file.replace(f'{path}/flac/','').replace('.flac', '')
    df.loc[df['fileName'] == filename, 'fileName'] = f'E01_01_19D_{i:07}'
